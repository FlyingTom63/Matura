...
writeLines(c(""), "~/Documents/workspaces/HSLU-ML/R/log.txt")  
sink("~/Documents/workspaces/HSLU-ML/R/log.txt", append=TRUE)
res <- iterateOverFeatures(data = main.data, samples = training.samples, X = X, k = 7)
closeAllConnections()
...
plot(mean, type="o", xlab = "Anzahl berücksichtigten Features", ylab = "Accurancy (Max)", xaxt = "n")
axis(1, at=1:9, labels=rev(c(10,9,8,7,6,5,4,3,2)))
