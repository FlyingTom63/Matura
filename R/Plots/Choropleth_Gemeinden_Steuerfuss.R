...
ggplot() +
  labs(
    title="Steuerbelastung nach Gemeinden gemäss Steuerprozenten",
    subtitle="5 Klassen, gleichverteilt, keine Missing Values",
    x=NULL,
    y=NULL) +
  geom_map (
    data = gd_dat,
...
ggplot() +
  labs(
     title="Steuerbelastung nach Gemeinden gemäss ",
     subtitle="Analyse der Rohdaten (Werte 1-1000, Missing Values -1000)",
     x=NULL,
     y=NULL) +
  geom_map (
     data = gd_dat,
     aes(map_id = BFS_NR, fill = Klasse),
     map = gd_map) + 
  expand_limits (
     x = gd_map$long,
     y = gd_map$lat) + 
